...
load("/data/joy/BBL/projects/xiaNetworkCca/sCCA/aim1/result/201701/restdti_regr_data.RData")
load("/data/joy/BBL/projects/xiaNetworkCca/sCCA/aim1/result/201701/restdti_med_rgr_data.RData")
net.data <- unname(combo.net.rgr)
med.data <- unname(restdti_train_med_rgr)
...
subjid <- restdti_train_qa

source('./../code/cca_functions.R')

# create 3 fold CV sets 10 times in the sub-training set

trainid <- createDataPartition(subjid$overall_psychopathology_4factor.x, p = 0.667, list =T,times=10)
...
p3Km111.cca<-mclapply(seq_along(trainid),function(i) ccaDW(brain_train[[i]],behavior_train[[i]],0.3,0.4,111))
...
p.var<-ggplot(varE,aes(modenum,ve_mean))+ 
  ...
  scale_y_continuous(expand = c(0,0),limits=c(0,0.045),labels = percent,name = "Variance Explained", breaks=seq(0,0.045,length=5)) +
...
p.cor<-ggplot(cor,aes(modenum,cor_mean,label=round(cor_mean,2)))+ 
  ...
  geom_text(size = 5, position = position_dodge(width=0.9), vjust= -1.8,color='grey')+
...
p3Km111.perm.cor<-sapply(behavior.perm, function(y_perm){ out<-ccaDWpermrank(data$brain,y_perm,0.3,0.4,k); out$cors} )
