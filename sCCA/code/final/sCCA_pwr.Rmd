...
```{r create sample splits}
load("./projects/xiaNetworkCca/sCCA/aim1/result/201701/pwr_train_qa.RData")
subjid <- pwr_train_qa
sampleid <- createDataPartition(subjid$overall_psychopathology_4factor, p = 0.667, list =T,times=10)
...
scca.org <- ccaDW(data$brain, data$behavior,0.8,0.4,modenum) #0.8 and 0.4 here are the best parameteres selected above in the grid search
...
scca.cand <- ccaDW(data$brain, data$behavior,0.8,0.4,candnum)
scca.cca<-mclapply(seq_along(sampleid),function(i) ccaDW(brain_sample[[i]],behavior_sample[[i]],0.8,0.4,20)) #loop through split
