...
```{r load data}
load("/data/joy/BBL/projects/xiaNetworkCca/sCCA/aim1/result/201701/gdn_train_net_ft.RData")
power.ft.train <- gdn_train_net_ft
```


```{r calculate covariance}
power_mad<- apply(power.ft.train,2,function(x) round(mad(x),digits=10))
...
gdn.rgr.train <- power.rgr
gdn.rgr.pca.train <- power.rgr.pca
save(gdn.rgr.pca.train,gdn.rgr.train,file='./result/201701/gdn_regr_data.RData')
```

```{r regress clinical features and regress}
load("/data/joy/BBL/projects/xiaNetworkCca/sCCA/aim1/result/201701/gdn_train_med.RData")
med <- gdn_train_med
...
gdn_train_med_rgr <- med.rgr

save(gdn_train_med_rgr, file = "./result/201701/gdn_med_rgr_data.RData")

```

```{r visulize which features}
power.data <- power.ft.train
power.cln.aj <- array(NA, c(333,333,dim(power.data)[1]))
...
levelplot(gdn.3k.train.ave, at = seq(-0.5,0.5,length=10),par.settings = BuRdTheme(),xlab='',ylab='',main="Gordon Train Set")

gdn.3k.train.ave <- power.cln.ave
gdn.3k.train.ave <- gdn.3k.train.ave[parcelshem$ParcelID,parcelshem$ParcelID]

gdn.3k.train.idx <- inc_idx
save(gdn.3k.train.ave,gdn.3k.train.idx,file ="./result/201701/gdn_3k_stats.RData")
```
