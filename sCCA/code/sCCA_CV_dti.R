...
load("/data/joy/BBL/projects/xiaNetworkCca/sCCA/aim1/result/201701/dti_regr_data.RData")
load("/data/joy/BBL/projects/xiaNetworkCca/sCCA/aim1/result/201701/dti_med_rgr_data.RData")
net.data <- unname(dti.rgr.train)
med.data <- unname(dti_train_med_rgr)
data <- list(brain = net.data, behavior = med.data)

## create sub-CV splits
load("/data/joy/BBL/projects/xiaNetworkCca/sCCA/aim1/result/201701/dti_train_qa.RData")
subjid <- dti_train_qa
...
p.cor<-ggplot(cor,aes(modenum,cor_mean,label=round(cor_mean,2)))+ 
  ...
  geom_text(size = 5, position = position_dodge(width=0.9), vjust= -2.5,color='grey')+
...
p3Km111.perm.cor<-sapply(behavior.perm, function(y_perm){ out<-ccaDW(data$brain,y_perm,0.8,0.4,k); out$cor} )
