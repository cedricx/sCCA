...
load("/data/joy/BBL/projects/xiaNetworkCca/sCCA/aim1/result/201701/bfc_med_rgr_data.RData")
net.data <- unname(pwr.rgr.train)
med.data <- unname(bfc_train_med_rgr)
...
y_pen <- seq(0.1,1,length.out=2)

p3Km111.gs<-ccaDWfoldgs(brain_train,behavior_train,x_pen,y_pen)
p3Km111.cca<-mclapply(seq_along(trainid),function(i) ccaDW(brain_train[[i]],behavior_train[[i]],p3Km111.gs$PENX,p3Km111.gs$PENY,5))
p3Km111.cca<-mclapply(seq_along(trainid),function(i) ccaDW(brain_train[[i]],behavior_train[[i]],0.8,1,5))

p3Km111.ve <- sapply(seq_along(trainid), function(i) p3Km111.cca[[i]]$d^2/sum(p3Km111.cca[[i]]$d^2))

# plot ve
# variance explained plots
varE <- data.frame( modenum = as.factor(1:5),ve_mean = rowMeans(p3Km111.ve),ve_se = rowSds(p3Km111.ve)/sqrt(dim(p3Km111.ve)[2]),vediff = c(abs(diff(rowMeans(p3Km111.ve))),0))
...
p.var<-ggplot(varE,aes(modenum,ve_mean))+ 
  ...
  scale_x_discrete(name ="Mode", limits=c(1:5),breaks =  seq(1,5,1)) +
  scale_y_continuous(expand = c(0,0),limits=c(0,0.3),labels = percent,name = "Variance Explained", breaks=seq(0,0.3,length=5)) +
...
cor <- data.frame(modenum = as.factor(1:5),cor_mean = rowMeans(p3Km111.cor), cor_se = rowSds(p3Km111.cor)/sqrt(dim(p3Km111.cor)[2]) )
...
p.cor<-ggplot(cor,aes(modenum,cor_mean,label=round(cor_mean,2)))+ 
  ...
  geom_text(size = 5, position = position_dodge(width=0.9), vjust= -1.2,color='grey')+
...
## bootstrap
bootid<-createResample(subjid$overall_psychopathology_4factor, list = T, times = 100)
