...
```{r load data}
load("./projects/xiaNetworkCca/sCCA/aim1/result/201701/pwr_regr_all_data.RData")
load("./projects/xiaNetworkCca/sCCA/aim1/result/201701/pwr_med_rgr_all_data.RData")

net.data <- unname(pwr.rgr.all)
med.data <- unname(pwr_all_med_rgr)
...
p3Km111.org <- ccaDW(data$brain, data$behavior,0.4,0.4,modenum)
```
...
p3Km111.cand <- ccaDW(data.train$brain, data.train$behavior,0.4,0.4,3)
```


```{r run sCCA on the sub-training splits}
p3Km111.cca<-mclapply(seq_along(trainid),function(i) ccaDW(brain_train[[i]],behavior_train[[i]],0.4,0.4,10))
p3Km111.cca.ro <- sapply(p3Km111.cca,function(x) reorderCCA(x,p3Km111.cand,10))
...
```{r plot correlation}
cor.df <- data.frame(modenum = as.factor(1:3), cor = p3Km111.cca.cor, se = p3Km111.cca.cor.se)
...
p3Km111.perm.cca<-sapply(behavior.perm, function(y_perm){ out<-ccaDWpermorder(data$brain,y_perm,0.4,0.4,candnum,p3Km111.cand)} )
...
load('./projects/xiaNetworkCca/sCCA/aim1/result/201701/sCCA_boot_train_ls1.RData')
sCCA.boot.train.ls.1 <- sCCA.boot.train.ls

p3Km111.boot <- append(sCCA.boot.train.ls.1,sCCA.boot.train.ls)
...
dim_mod_plot <- lapply(1:3,function(dim) lapply(as.list(netmod_sig_idx[[dim]]),function(mod) within_mod_plot(br.signed[[dim]]$mat,mod)))

dim_mod_calc <- lapply(1:3,function(dim) sapply(as.list(netmod_sig_idx[[dim]]),function(mod) within_mod_calc(br.signed[[dim]]$mat,mod)))
...
dim_bt_mod_plot <- lapply(seq_along(1:3), function(i) between_mod_load_plot(netmod_bt[[i]],dim_mod_calc[[i]],paste("Dimension",i)))
