...
```{r load data}
load("./projects/xiaNetworkCca/sCCA/aim1/result/201701/pwr_all_net_ft.RData")
power.ft.train <- pwr_all_net_ft
```
...
power_mad 
inc_idx <- which(power_mad>=power_mad_order$mad[3000])
...
power.rgr <- apply(inc_net, 2, function(x) residuals.glm(glm(x ~  
    ageAtScan1 + sex + race2 + restRelMeanRMSMotion, data = sample_qa), type = "response"))

# PCA-on these data
power.rgr.pca <-prcomp(power.rgr, center = FALSE)

pwr.rgr.all <- power.rgr
pwr.rgr.pca.all <- power.rgr.pca


save(pwr.rgr.pca.all,pwr.rgr.all,file='./result/201701/pwr_regr_all_data.RData')
...
pwr_all_med_rgr <- med.rgr

save(pwr_all_med_rgr, file = "./result/201701/pwr_med_rgr_all_data.RData")
