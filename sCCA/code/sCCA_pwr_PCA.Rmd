...
net.data <- unname(pwr.rgr.pca.train$x[,1:100])
...
p110PC.gs <- lapply(1:10, function(i) CCA.permute(data$brain,data$behavior,penaltyxs = seq(0.1,1,length.out = 10), penaltyzs = seq(0.1,1,length.out = 10)[i],nperms = 100))
z_stats <- sapply(foo.perm, function(x) x$zstats)
p_vals <- sapply(foo.perm, function(x) x$pvals)
```

```{r run sCCA on all data }
modenum <- dim(data$brain)[2]
p110PC.org <- ccaDW(data$brain, data$behavior,1,0.4,modenum)
```

```{r select components based on covariance matrix}
brain_std <- apply(data$brain,2,scale)
med_std <- apply(data$behavior,2,scale)
covmat <- t(p110PC.org$u) %*% t(brain_std) %*% med_std %*% p110PC.org$v
...
p.var<-ggplot(varE.df,aes(modenum,var)) +
  ...
  scale_y_continuous(expand = c(0, 0),limits=c(0,0.1),labels = percent,name = "Variance Explained", breaks=seq(0,0.1,length=4)) +
...
candnum <- 3
p3Km111.cand <- ccaDW(data$brain, data$behavior,1,0.4,candnum)
```

```{r run sCCA on the sub-training splits}
p3Km111.cca<-mclapply(seq_along(trainid),function(i) ccaDW(brain_train[[i]],behavior_train[[i]],1,0.4,10))
...
p3Km111.perm.cca<-sapply(behavior.perm, function(y_perm){ out<-ccaDWpermorder(data$brain,y_perm,1,0.4,candnum,p3Km111.cand)} )
...
#ubyx_std_Ave
brain_std_mat_ave <- apply(brain_std_mat,c(1,2),mean)
...
dim_bt_mod_plot <- lapply(seq_along(1:4), function(i) between_mod_load_plot(netmod_bt[[i]],dim_mod_calc[[i]],paste("Dimension",i)))
